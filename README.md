...
6. **웹사이트 최적화 배치 실행** 🔍
   - **SEO 분석**: 메타 태그, 헤딩, robots.txt, sitemap.xml 체크
   - **AI SEO**: AI 기반 키워드 리서치 및 콘텐츠 최적화 초안 생성
   - **GEO (Generative Engine Optimization)**: 생성형 엔진 친화 구조 및 FAQ 스키마 최적화
   - **AIO 종합 최적화**: SEO/AI SEO/GEO/콘텐츠 일괄 검증 및 통합 분석
   - 도메인 입력 한 번으로 모든 최적화 분석을 순차 실행하고 상세 보고서 제공
   - 자세한 내용은 [dev-agent-kit 통합 가이드](./docs/dev-agent-kit-integration.md) 참고
